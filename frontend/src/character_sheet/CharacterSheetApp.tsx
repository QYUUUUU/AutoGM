...
export default function CharacterSheetApp({ initialCharacter }: { initialCharacter: any }) {
  ...
  return (
    // Standardized root wrapper: pt-16 for header offset, min-h-screen
    <div className="gods:pt-16 gods:min-h-screen gods:bg-background gods:relative gods:z-10 gods:flex gods:flex-col">

      {/* Standardized layout main block: w-full max-w-7xl + responsive py */}
      <main className="gods:flex-1 gods:w-full gods:max-w-7xl gods:mx-auto gods:px-6 gods:py-12 lg:gods:px-12 lg:gods:py-24">
        
        <header className="gods:mb-12 gods:border-b gods:border-border/50 gods:pb-8">
          <div className="gods:flex gods:flex-wrap gods:items-start gods:justify-between gods:gap-4">
            <div>
              <SectionLabel>Fiche de personnage</SectionLabel>
              <PageTitle>{character.nom || "Personnage"}</PageTitle>
              <PageDescription>Caractéristiques, compétences, santé et réserves</PageDescription>
            </div>
            
            {/* Status indicator aligned to the top-right of the header flex container */}
            <span className={`gods:mt-2 gods:flex gods:items-center gods:gap-2 gods:text-xs gods:font-display gods:tracking-widest ${saveStatus === "saved" ? "gods:text-primary" : "gods:text-muted-foreground"}`}>

...
// Fix character.Nom
ui = ui.replace(/character\.Nom/g, 'character.nom');

// Add modal logic for upgrading and handle stat selection
const modalHTML = `
<!-- Eclat Upgrade Modal -->
...
// Update router route for upgrade
...
// Replace the upgrade block
routes = routes.replace(
    /router\.post\('\/pacte\/upgrade', isAuthenticated, async \(req, res\) => \{[\s\S]*?catch\(err\) \{[\s\S]*?res\.status\(500\)\.json\(\{ success: false, message: 'Erreur Serveur\.' \}\);\n\s+\}\n\}\);/,
